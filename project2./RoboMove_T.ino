...
// -- 속도 제어 파라미터 (실험적으로 변경) --
const int MAX_PWM = 255;  // 최대 PWM (초기 PWM)
const int MIN_PWM = 50;   // 정지 시 마찰 극복 -> 최소 PWM (실험적으로 결정) -> 편차 보정보다 크게 설정하여 감속 프로파일이 적용되도록 함
...
void setup()
{
    ...
    Serial1.begin(Raspberry); // 라즈베리파이 통신
    ...
    while (Serial1.available())
    {
        ...
        if (c == '\n')
        {
            ...
            if (rxBuf.length() > 0)
            {
                ...
                {
                    // "T steer"
                    float steer = constrain(rxBuf.substring(2).toFloat(), -1.0f, 1.0f);
                    driveContinuous(steer, 0.5f, false); // 제자리 회전 (후진)
                }
                else if (cmd == 'S')
                {
                    brakeMotors();
