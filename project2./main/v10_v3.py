        ...
        try:
            ...
            if not any(has_pt):
                ...
                if best is not None and best['passable'] and abs(best['center']) <= ROT_THRESH:
                    ...
                    # 측면 벽 반발력: 좌(270°)/우(90°) 벽이 가까울수록 반대쪽으로 밀어냄
                    WALL_REP  = 350.0
                    ...
                    repulsion = (rep_L - rep_R) * 18.0   # 도 단위, 가까운 벽에서 밀어냄

                    target    = best['center'] + bias + repulsion
                    ...
                    print(f"VFH_FWD  갭={best['width']:.0f}mm@{best['center']:+.0f}도  "
                            f"bias={bias:+.1f}도  rep={repulsion:+.1f}도  근접={near_d:.0f}mm  "
                            f"L={lat_L:.0f} R={lat_R:.0f}  steer={steer:+.2f}  spd={speed:.2f}")
