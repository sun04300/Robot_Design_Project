...
def find_vfh_gaps(hist, has_pt, detect_dist, min_pass_mm):
    ...
    Returns: list of gap dict
      ...
      d_left    : 갭 왼쪽 엣지 장애물 거리 (mm)
      d_right   : 갭 오른쪽 엣지 장애물 거리 (mm)
      delta_deg : 갭 각도 폭 (도)
    """
...
def select_best_gap(gaps):
    ...
    return max(pool, key=lambda g: g['width'] * 0.3 - abs(g['center']))
...
while True:
    ...
    try:
        ...
        _ready    = True
        scan_buf  = []
        # 긴급후진 판단용 narrow zone (VFH 이전에 빠르게 체크)
    ...
    if s_flag == 1 and len(scan_buf) > 15:
        ...
        if not any(has_pt):
            ...
            # 전방 EMERGENCY 이내 + 전방 방향 통과 가능 갭 없을 때만 후진
            # 전방 갭이 있으면 P3에서 처리 (후진 불필요)
            ...
            # ── P3: VFH 전진 — 갭이 전방 반구(+-ROT_THRESH) ─
                ...
                # gap['center']: +오른쪽 / -왼쪽
                # steer = center/90 x MAX_STEER
                #   center > 0 -> 양수 steer (오른쪽)
                #   center < 0 -> 음수 steer (왼쪽)
                steer  = max(-MAX_STEER, min(MAX_STEER,
                             best['center'] / 90.0 * MAX_STEER))

                # 속도: 갭 방향 +-20도 내 최근접 장애물 기준 감속
            ...
            # 갭이 +-ROT_THRESH 너머(후방)에 있을 때
            # T 명령으로 제자리 회전 -> 후진 없이 방향 전환
            ...
            # ── P5: 통과 가능한 갭 없음 -> 최후 수단 후진 ─
            # VFH 기반 경로에서는 거의 발생하지 않음
            # (DETECT=500mm로 미리 반응하기 때문)
            else:
                back_cnt += 1
                ser_Ardu.write(b"B 0.70\n")
                widest = max((g['width'] for g in gaps), default=0.0)
                print(f"VFH_BACK  통과 가능 갭 없음 "
                      f"(최대폭={widest:.0f}mm < {GAP_MIN_PASS:.0f}mm)  ({back_cnt})")
                if back_cnt >= 6:
                    extra_back = 2
                    back_cnt   = 0
                    print("EXTENDED_BACK 시작! (3x)")
