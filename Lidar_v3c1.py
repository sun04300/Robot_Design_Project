...
while True:
    ...
    try:
        ...
        # [수정] 코너 감지 조건 완화 및 후진 공간 대폭 확대
        CORNER_FRONT    = EMERGENCY * 1.5   # 감지 거리 상향 (약 210mm)
        CORNER_BLOCKED  = 4                 # 6개 -> 4개 섹터만 막혀도 코너로 의심
        BACKUP_CYCLES    = 8                # 3사이클 -> 8사이클로 대폭 늘려 회전 반경 확보
        ...
        print("=" * 55)
        print("  장애물 회피 v3c_Fixed: 코너 교착 무한루프 탈출 버전")
        print(f"  감지: {int(DETECT)}mm  /  긴급후진: {int(EMERGENCY)}mm")
        print(f"  코너기준: 전방 ≤ {int(CORNER_FRONT)}mm AND ({CORNER_BLOCKED}섹터 이상 또는 3면 차단)")
        print(f"  회전종료(폐루프): 전방 {int(FRONT_CLEAR)}mm 이상 (협소+광역 AND 만족)")
        print(f"  전진확정: {COMMIT_CYCLES}사이클  /  탈출후진: {BACKUP_CYCLES}사이클")
    ...
    if s_flag == 1 and len(scan_buf) > 15:
        ...
        # [수정] 4개 이상 섹터가 막혔거나, 좁은 반경 내 전/좌/우가 동시에 가로막힌 경우 코너 판정
        ...
        # 광역 전방 섹터 평균 (섹터 0, 섹터 7)
        front_sectors_min = min(sector_avg[0], sector_avg[NUM_SECTORS - 1])

        # [수정] 탈출 성공 조건 강화: 협소 전방구역과 광역 전방구역이 '모두(AND)' 완전히 뚫렸을 때만 전진 전환
        ...
        signal_source = ""
        if narrow_clear and wide_clear:
            front_clear_signal = True
            signal_source = f"Both_Clear(N:{front_min:.0f}, W:{front_sectors_min:.0f})"

        # ── 우선순위 1: 탈출 회전 (광역 폐루프) ──────────────────────
        if escape_left > 0:
            # [수정] 회전 도중 측면/전방 벽에 다시 끼이는 현상 방지용 안전 가드
            if front_min <= EMERGENCY or left_min <= EMERGENCY or right_min <= EMERGENCY:
                escape_left = 0
                extra_back = 4  # 탈출 취소 후 즉시 재후진 공간 확보
                ser_Ardu.write(b"B 0.70\n")
                print("ESCAPE_ABORT! 회전 중 장애물 재근접 → 탈출 일시중지 및 재후진")
            elif front_clear_signal:
                escape_left    = 0
