...
while True:
    ...
    try:
        ...
        # [v3a 신규] 코너 감지를 위한 엄격한 기준
        CORNER_FRONT    = EMERGENCY * 1.4   # ? 196mm ? 진짜 벽 코앞일 때만
        CORNER_BLOCKED  = 6                 # 8섹터 중 6개 이상 막힘 (이전: 4)
        back_cnt        = 0
        extra_back      = 0
        ...
        print("=" * 55)
        print("  장애물 회피 v3a: is_cornered 기준 강화")
    ...
    if s_flag == 1 and len(scan_buf) > 15:
        ...
        # [v3a 변경] 코너 판정 기준 강화
        # 이전: front_min <= DETECT(330) and blocked >= 4
        # 이후: front_min <= CORNER_FRONT(196) and blocked >= CORNER_BLOCKED(6)
        ...
        if escape_left > 0:
            if front_cnt >= MIN_COUNT and front_min >= FRONT_CLEAR:
                ...
                print(f"ESCAPE_DONE  전방 {front_min:.0f}mm 확보 → 전진 복귀")
            else:
